...
def train(data_path, patch_size=50, mini_batch=512, nms_th=0.05, line_positive_th=0.05, line_negative_th=0.10, loss_weight=[1.0, 1.0, 1.0], sigma=0.01, clip=0.02):
    ...
    if recover_from_last_train:
        backbone_net.load_state_dict(torch.load('./checkpoint_sigma{}clip{}_building3d/backbone_patchSize{}_miniBatch{}_nmsTh{}_linePosTh{}_lineNegTh{}_lossweightP{}V{}L{}.pth'.format(
    ...
    if recover_from_last_train:
        patch_net.load_state_dict(torch.load('./checkpoint_sigma{}clip{}_building3d/patchnet_patchSize{}_miniBatch{}_nmsTh{}_linePosTh{}_lineNegTh{}_lossweightP{}V{}L{}.pth'.format(
            sigma, clip, patch_size, mini_batch, nms_th, line_positive_th, line_negative_th, loss_weight[0], loss_weight[1], loss_weight[2]
        )))
    vertex_net = vertexNet().cuda()
    if recover_from_last_train:
        vertex_net.load_state_dict(torch.load('./checkpoint_sigma{}clip{}_building3d/vertexnet_patchSize{}_miniBatch{}_nmsTh{}_linePosTh{}_lineNegTh{}_lossweightP{}V{}L{}.pth'.format(
            sigma, clip, patch_size, mini_batch, nms_th, line_positive_th, line_negative_th, loss_weight[0], loss_weight[1], loss_weight[2]
        )))
    line_net = lineNet().cuda()
    if recover_from_last_train:
        line_net.load_state_dict(torch.load('./checkpoint_sigma{}clip{}_building3d/linenet_patchSize{}_miniBatch{}_nmsTh{}_linePosTh{}_lineNegTh{}_lossweightP{}V{}L{}.pth'.format(
            sigma, clip, patch_size, mini_batch, nms_th, line_positive_th, line_negative_th, loss_weight[0], loss_weight[1], loss_weight[2]
        )))
    ...
    for epoch in range(n_epoch):
        ...
        for train_loader_i, data in enumerate(tqdm(train_loader)):
            ...
            # print("pc_down length:"+str(len(pc_down)))
            try:
