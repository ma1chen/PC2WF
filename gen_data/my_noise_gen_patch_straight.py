...
class GenPatch:
    def __init__(self, point_path, patch_size=50, sigma=0.01, clip=0.02, train_val_test='test'):
        try:
            ...
            base_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../roof3d_data/patches_{}_noise_sigma{}clip{}/{}'.format(patch_size, sigma, clip, train_val_test))
            ...
            self.pointcloud = np.loadtxt(point_path, usecols=(0, 1, 2))
            ...
            for idx, patch in enumerate(patch_vert):
                ...
                print(f"dist_vert shape: {len(dist_vert)}")
            ...
            print("generate patch with vert successfully")
            ...
            print("generate other patch successfully")
            ...
            print(f"Error processing file {point_path}: {e}")
            raise
    ...
    def farthest_sample(self, k):
        farthest_pts_index = np.zeros((k, ), dtype=np.int32)  # 创建一个大小为 k 的数组用于存储最远点的索引
        farthest_pts = np.zeros((k, 3)) # 创建一个大小为 (k, 3) 的数组用于存储最远点的坐标
        farthest_pts_index[0] = np.random.randint(len(self.pointcloud_down)) # 随机选择一个点作为第一个最远点的索引
        # 添加调试信息
        print(f"self.pointcloud_down.shape: {self.pointcloud_down.shape}")
        print(f"farthest_pts_index[0]: {farthest_pts_index[0]}")
        print(f"self.pointcloud_down[farthest_pts_index[0]].shape: {self.pointcloud_down[farthest_pts_index[0]].shape}")
        farthest_pts[0] = self.pointcloud_down[farthest_pts_index[0]] # 存储第一个最远点的坐标
    ...
    def load_gt(self):
        ...
        with open(gt_path, 'r') as gt_f:
            ...
            while item_gt:
                ...
                print(f"Reading line: {item_gt}")  # 打印每一行的内容
                if item_gt[0] == 'v':
    ...
    def down_sample(self):
        ...
        # 如果点云数量小于一万，直接保存
        if self.pointcloud.shape[0] < 10000:
            pointcloud_down = self.pointcloud
            feats_down = feats
            coords = np.hstack([self.pointcloud, np.zeros((self.pointcloud.shape[0], 1))])
        else:
            # 否则进行下采样
            ...
            feats_down = feats[inds]

        # 保存路径
        point_down_path = self.save_root_path.replace('.xyz', '.down')
        # 保存下采样后的点云
        np.savetxt(point_down_path, pointcloud_down, fmt='%0.6f')
        # 重新加载下采样后的点云（可选步骤，原代码有此操作）
        pointcloud_down = np.loadtxt(point_down_path, usecols=(0, 1, 2))
        # 保存特征矩阵
        np.savetxt(self.save_root_path.replace('.xyz', '.feats'), feats_down)
        # 保存坐标
...
if __name__ == '__main__':
    ...
    for dirname in ['train', 'test', 'validation']:
        file_list = glob(os.path.join(os.path.dirname(os.path.abspath(__file__)), '../roof3d_data/noise_sigma{}clip{}/xyz/{}/*.xyz'.format(sigma, clip, dirname)))
        file_list.sort()

        base_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../roof3d_data/patches_{}_noise_sigma{}clip{}/{}'.format(patch_size, sigma, clip, dirname))
